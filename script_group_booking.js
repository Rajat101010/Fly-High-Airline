...
function addPassenger() {
  ...
  validateForm(); // Check after adding new passenger
}

function validateForm() {
  const requiredFields = bookingForm.querySelectorAll('[required]');
  const passengers = document.querySelectorAll('#passengerContainer .passenger');
  ...
  const hasMinPassengers = passengers.length >= 10;
  searchBtn.disabled = !(allFilled && hasMinPassengers);
}
